...
%type <program> Program Procedures ProcedureDecl
%type <block> ProcedureBody 
%type <functiondecl> ProcedureHead FunctionDecl
...
%type <varvec> DeclList IdentifierList 
...
Procedures 	: ProcedureDecl Procedures
		{
		  $2->blocks.insert($2->blocks.begin(),$1->blocks[0]);
		   $$ = $2;
		}
	   	|
		{
...
ProcedureDecl : ProcedureHead ProcedureBody
		{
			...
			for(int i=0;i<$1->variables.size();i++){
			  $2->localVariables.push_back($1->variables[i]);
			}
			$2->name = $1->name;
			$$->blocks.push_back($2);
		}
...
FunctionDecl :  Type IDENTIFIER LPAREN RPAREN LBRACE 
		{
		  context->functionName = $2->name;
		  $$ = new NFunctionDeclaration($2->name);
		  $$->name = $2->name;
		}
	      	;
...
VarDecl 	: IDENTIFIER
		{ 
		  ...
		  if(context->functionName=="global"){
		    ...
		    context->localvaroffset[context->functionName][$1->name]=context->currentlocaloffset[context->functionName];
		...
		| IDENTIFIER LBRACKET INTCON RBRACKET
			...
			if(context->functionName=="global"){			
			  ...
			  context->currentlocaloffset[context->functionName]+=sizeof(int)*($1->capacity); 
		    context->localvaroffset[context->functionName][$1->name]=context->currentlocaloffset[context->functionName];
