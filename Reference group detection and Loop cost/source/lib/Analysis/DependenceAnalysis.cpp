...
bool DependenceAnalysis::runOnFunction(Function &F) {
  ...
  */
  ...
  for (inst_iterator SrcI = inst_begin(F), SrcE = inst_end(F);
    ...
    // get the loop number
    // the basic idea is get the leve1 of a loop (outermost)
    // find its parent and save it 
    // when find level 1 again check two loop if same
    // if not same which means get another loop nest
    {
      const BasicBlock *DstBlock = SrcI->getParent();
      const Loop *DstLoop = LI->getLoopFor(DstBlock);
      
      unsigned DstLevel = LI->getLoopDepth(DstBlock);
      if (DstLevel == 1 && !DstLoop->getParentLoop()){
	if(currentLoop != DstLoop){
	  currentLoop = DstLoop;
	  loopNest++;	  
	...
	if (SrcGEP) {  
	  const SCEV *SrcPtrSCEV = SE->getSCEV(SrcGEP->getPointerOperand());
	  UsefulGEP =
	    isLoopInvariant(SrcPtrSCEV, LI->getLoopFor(SrcI->getParent()));

	  // how many subscripts does an array have: Pairs-1  
	unsigned Pairs = UsefulGEP ? SrcGEP->idx_end() - SrcGEP->idx_begin() : 1;
	SmallVector<Subscript, 4> Pair(Pairs);
	if (UsefulGEP) {
	  unsigned P = 0;
	  for (GEPOperator::const_op_iterator SrcIdx = SrcGEP->idx_begin(), 
		 SrcEnd = SrcGEP->idx_end();
	       SrcIdx != SrcEnd;
	       ++SrcIdx, ++P) {
	    Pair[P].Src = SE->getSCEV(*SrcIdx);	    // save all relative values to a vertor
	  }
	}
	else {
	  const SCEV *SrcSCEV = SE->getSCEV(SrcPtr);
	...
	// From this vector , we could deal with all subscripts in different level
	for (unsigned P = 0; P < Pairs; ++P) {  
	...
	int SizeOfVariable;
	// get the type of variables, for example int or double,and get the right size of different variables.
	// for example a chache line size 64 could hold 16 int but 8 doulbe variables.
	// if we get a pointer type, make sure we get the pointerelement instead.
 	if(SrcI->getOperand(0)->getType()->isPointerTy()){
	  SizeOfVariable = AA->getTypeStoreSize(SrcI->getOperand(0)->getType()->getPointerElementType());
	}
	else{
	...
	APInt strideCount = APInt(64,0); 
	APInt stride = APInt(64,0);
	...
	for (unsigned P = 0; P < Pairs; ++P) {  
	  ...
	  if(SrcAddRec){ 
	    // deal with MIV situations. a[i][k+j][k]
	    do{
	      SrcConst = SrcAddRec->getStart();
	      SrcCoeff = SrcAddRec->getStepRecurrence(*SE);
	      SrcLoop =  SrcAddRec->getLoop();
		...
		if(P == Pairs-1){
		  strideCount =strideCount+stride;
		}
	      }
	      SrcAddRec =dyn_cast<SCEVAddRecExpr>(SrcAddRec->getStart());
	    }
	    while(SrcAddRec); 

	    // statics at the innermost level
	    if(P == Pairs-1){
	      // make sure the total stride count is lower than cache line count,
	      // als it could not be zero, like a[i][i] in j loop, only self-temp reuse
	      if(strideCount.slt(CacheLineCount) && strideCount.isStrictlyPositive () ){
		level = Pairs-1;
		selfspatial = true;
	      }	    
	    }	    
