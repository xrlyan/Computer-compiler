...
//STATISTIC(NumSVNInstr,  "Number of instructions deleted by SVN");
//STATISTIC(NumLVNInstr,  "Number of instructions deleted by LVN");
...
// add by liang for lvn and svn
static cl::opt<bool> EnableLVN("enable-lvn",
               cl::init(false), cl::Hidden);
static cl::opt<bool> EnableSVN("enable-svn",
                              cl::init(false), cl::Hidden);
// end by liang
...
// TODO, try to seperate SVN and LVN from GVN
// so far, I already put the commands in std-compile-opts
// and could say it from help,
// not implement its runOnFunction part.
#if 1
namespace{
  //    class LVN : public FunctionPass {
    ...
    LVN():FunctionPass(ID) {initializeLVNPass(*PassRegistry::getPassRegistry());
    }
    virtual bool runOnFunction(Function &F) {
      //       NumLVNInstr=NumGVNInstr;
      return false;
     } 
  };
char LVN::ID = 0;
}
INITIALIZE_PASS(LVN, "lvn","Local value numbering", false, false)

FunctionPass *llvm::createLVNPass() {
  return new LVN();
}

namespace{
  //    class SVN : public FunctionPass {
    class SVN : public FunctionPass {
    public:
    static char ID; // Pass identification, replacement for typeid
    SVN():FunctionPass(ID) {initializeSVNPass(*PassRegistry::getPassRegistry());
    }
    virtual bool runOnFunction(Function &F) {
      //     NumSVNInstr=NumGVNInstr;
      return false;
     } 
  };
}
char SVN::ID = 0;
INITIALIZE_PASS_BEGIN(SVN, "svn", "Super local Value Numbering", false, false)
INITIALIZE_PASS_DEPENDENCY(MemoryDependenceAnalysis)
INITIALIZE_PASS_DEPENDENCY(DominatorTree)
INITIALIZE_PASS_DEPENDENCY(EBBForest)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfo)
INITIALIZE_AG_DEPENDENCY(AliasAnalysis)
INITIALIZE_PASS_END(SVN, "svn", "Super local Value Numbering", false, false)

FunctionPass *llvm::createSVNPass() {
  return new SVN();
}
...
namespace {
  ...
  public:
    ...
    EBBForest &getEBBForest() const { return *EBBS; }
  ...
  private:
    ...
    virtual void getAnalysisUsage(AnalysisUsage &AU) const {
      ...
      // add by liang
      AU.addRequired<EBBForest>(); // get the instance
      AU.addPreserved<EBBForest>(); // do not change its value, like const
      // end by liang 
...
INITIALIZE_PASS_DEPENDENCY(MemoryDependenceAnalysis)
INITIALIZE_PASS_DEPENDENCY(DominatorTree)
INITIALIZE_PASS_DEPENDENCY(EBBForest) // add by liang
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfo)
INITIALIZE_AG_DEPENDENCY(AliasAnalysis)
...
Value *GVN::findLeader(const BasicBlock *BB, uint32_t num) {
  ...
  // the is the entry for delete an instruction by value numbering
  Value *Val = 0;	
  if(!EnableLVN&&!EnableSVN){ 
  ...
  }else if(EnableSVN) // check the block it is extend by and itself in an EBB
  {
    if (EBBS->EBB.isAncestor(Vals.BB, BB)) {
      Val = Vals.Val;
      if (isa<Constant>(Val)) return Val;
    }
    ...
    LeaderTableEntry* Next = Vals.Next;
    while (Next) {
      if (EBBS->EBB.isAncestor(Next->BB, BB)) {
	if (isa<Constant>(Next->Val)) return Next->Val;
	if (!Val) Val = Next->Val;
      }
      Next = Next->Next;
  ...
  else{ // only check the same basic block
    if (Vals.BB == BB) {
      Val = Vals.Val;
      if (isa<Constant>(Val)) return Val;
    }
...
bool GVN::runOnFunction(Function& F) {
  ...
  // add by liang
  // get an insistance of class EBBForest after the EBB pass
  EBBS = &getAnalysis<EBBForest>(); 
  // end by liang 
  ...
  // add by liang.
  // No necessary for local and super local to 
  //merge the block, although it does not affect the result.
  if(!EnableLVN || !EnableLVN){
  ...
  // add by liang. No necessary for local and super local to 
  //enable PRE.
  if(!EnableLVN || !EnableLVN){
...
bool GVN::iterateOnFunction(Function &F) {
   ...
   // add by liang
   // this is the entry for basic block. I will need to decide 
   // which blocks should be used according to different 
  ...
  for (std::vector<BasicBlock *>::iterator I = BBVect.begin(), E = BBVect.end();
    ...
    // the normal BB could work well for local value numbering
    for (Function::iterator FI = F.begin(), FE = F.end(); FI != FE; ){
      BasicBlock *BB = FI++;
      Changed |= processBlock(BB);      
      cleanupGlobalSets();
    }
  }else{ // get the ebb information, iterate each ebb.
    EBBS->setFunctionName(F.getName());
    std::vector<EBBTree<BasicBlock> > ebbs = EBBS->ForestTable[F.getName()];

    for(std::vector<EBBTree<BasicBlock> >::iterator i = ebbs.begin(), e = ebbs.end(); i != e; ++i){
      std::vector<BasicBlock*> currentTree= i->getTree();
      EBBS->EBB.Ebb= currentTree;
      for(std::vector<BasicBlock*>::iterator j = currentTree.begin(), k = currentTree.end(); j != k; ++j){
	BasicBlock *currentBlock = *j;
	Changed |= processBlock(currentBlock);
      }
      cleanupGlobalSets();//flush the table when finishing an EBB.
    }
  }
  // end by liang
  return Changed;
